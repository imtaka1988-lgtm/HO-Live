...
let eventTableReady = false;

async function ensureEventTable(pool) {
  if (eventTableReady) return;
  await pool.query(
  ...
  eventTableReady = true;
}

function allowUnsignedCallback() {
  const v = String(process.env.LIVE_CALLBACK_ALLOW_UNSIGNED || "").toLowerCase();
  return v === "1" || v === "true" || v === "yes";
}

function callbackKeyError(req) {
  const expected = String(process.env.LIVE_CALLBACK_KEY || "");
  if (!expected) return allowUnsignedCallback() ? "" : "callback key missing";

  const got = String(
    req.query.key ||
    req.headers["x-live-callback-key"] ||
    ""
  );
  if (got !== expected) return "callback key invalid";
  return "";
}
